...
Data_memory #(.INIT_FILE("/home/luchosteam/datos.txt")) Data_memory(.Wr(WrRAM),.clk(clk),.ena(BIP_enable), .Addr(Addr), .In_Data(In_Data), .Out_Data(Out_Data));
Program_memory #(.INIT_FILE("//home/luchosteam/instrucciones.txt")) Program_memory (0, clk, BIP_enable, PC, 0, Program_Data);
